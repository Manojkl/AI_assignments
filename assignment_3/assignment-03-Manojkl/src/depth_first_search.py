...
if __name__ == '__main__':
    ...
    for i in target:
        # Extracting the path
        path_map1 = depth_first_search(maze_map_map1, index_of_s, i)
    ...
    all_maps = []
    for i in target:
        path_map2 = depth_first_search(maze_map_map2, index_of_s, i)
        if path_map2 != None:
            maps =write_to_file(path_map2, maze_map_map2)
            all_maps.append(maps)
    
    with open('dfs_map2.txt', 'w') as f:
            for i in all_maps:
                for item in i:
                    f.write("%s\n" % item)
    ...
    all_maps = []
    for i in target:
        path_map3 = depth_first_search(maze_map_map3, index_of_s, i)
        if path_map3 != None:
            maps = write_to_file(path_map3, maze_map_map3)
            all_maps.append(maps)
    
    with open('dfs_map3.txt', 'w') as f:
            for i in all_maps:
                for item in i:
                    f.write("%s\n" % item)
